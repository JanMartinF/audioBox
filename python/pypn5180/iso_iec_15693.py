...
class iso_iec_15693(object):
    ...
    def getSystemInformationCmd(self, uid=[]):
        ...
        # Add UID if the AFI flag is set
        if uid is not []:
            frame.extend(uid)
        ...
        if 'OK' in error and len(data) >= 13:
            # (Number of blocks - 1) is at index 11
            num_blocks = data[11] + 1
            # (Number of bytes per block - 1) is at index 12
            block_size = data[12] + 1
            return data, error, num_blocks, block_size
